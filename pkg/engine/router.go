...
var (
	...
	// these bhojpur.Controller's methods shouldn't reflect to AutoRouter
...
func (l *logFilter) Filter(ctx *ctxutl.Context) bool {
	...
	for prefix := range BasConfig.WebConfig.StaticDir {
...
func (p *ControllerRegister) addWithMethodParams(pattern string, c ControllerInterface, methodParams []*param.MethodParam, mappingMethods ...string) {
	...
	route.routerType = routerTypeBhojpur
...
func (p *ControllerRegister) getURL(t *Tree, url, controllerName, methodName string, params map[string]string, httpMethod string) (bool, string) {
	...
	for _, l := range t.leaves {
		if c, ok := l.runObject.(*ControllerInfo); ok {
			if c.routerType == routerTypeBhojpur &&
