...
func TestJsonStartsWithArray(t *testing.T) {
	...
	{
		"url": "user",
		"serviceAPI": "http://app.bhojpur.net/user"
	},
	{
		"url": "employee",
		"serviceAPI": "http://app.bhojpur.net/employee"
	}
]`
	...
	if rootArrayCasted == nil {
		...
		if elem["url"] != "user" || elem["serviceAPI"] != "http://app.bhojpur.net/user" {
			t.Error("array[0] values are not valid")
		}

		elem2 := rootArrayCasted[1].(map[string]interface{})
		if elem2["url"] != "employee" || elem2["serviceAPI"] != "http://app.bhojpur.net/employee" {
